...
# transformed: FALSE [if the data are already clr transformed or normalised]
# lib_size: matrix of size n x t
# single: FALSE [longitudinal version of LUPINE]

LUPINE<- function(data, day, excluded_taxa=NULL, transformed=FALSE, lib_size=NULL, single=FALSE){
  ...
  if(!transformed){
    ...
    if(single){
      ...
      for(i in 1:len){
        #print(i)
        loading_tmp<-loadings_m
        u1<-scale(count_day_f.clr, center=TRUE, scale=TRUE)%*%loading_tmp
