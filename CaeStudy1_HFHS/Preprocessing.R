...
all.fil.otus<-t(apply(filtered_bio_OTU,1, function(x) x/sum(x))) %>%
  data.frame() %>%
...
highabundance<-pivot_wider(all.fil.otus, names_from = sOTUs, values_from = MeanRA)%>%
  select(c("Diet", "Day",OTU_healthy_sort$OTU))
OTU_l.abundance<-apply(X = is.na(highabundance),MARGIN = 1,
                       FUN = function(x) colnames(highabundance)[x])
low_HFHS<-OTU_l.abundance[1:4]
low_Normal<- OTU_l.abundance[5:8] 
...
save(low_HFHS, file="data_filtered/low_HFHS.rds")
save(low_Normal, file="data_filtered/low_Normal.rds")
