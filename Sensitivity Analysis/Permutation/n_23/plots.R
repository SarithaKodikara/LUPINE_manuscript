...
df_bpls<-df_sen_pre
load("Results/df_sen_pre_p_perm.rds")
df_pca_perm<-df_sen_pre
load("Results/df_sen_pre_b_perm.rds")
df_bpls_perm<-df_sen_pre
###plots for PLS

df_sen_pre_all<- rbind(df_pca,df_pca_perm, df_bpls, df_bpls_perm)
...
pdf("Figures/n23_permutation.pdf",
    width=8, height=8)
set.seed(12345)
op <- par(mar = rep(0, 4))
