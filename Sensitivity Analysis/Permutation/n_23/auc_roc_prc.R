...
auc_res<-function( iter, day, method='sparcc'){
  ...
  }else if(method=='PCA_perm'){
    elap_time<-system.time({
      pcor_perm.pval<-matrix(NA, nrow = nOTU,ncol = nOTU)
      ...
      sapply(1:len, function(i){
        ...
        r_i<-lm(log(newSimData[[iter]][,taxa1[i], day]+1)~u1,offset=log(newLibSize[[iter]][,day]))
        r_j<-lm(log(newSimData[[iter]][,taxa2[i], day]+1)~u1,offset=log(newLibSize[[iter]][,day]))
        pcor_perm.pval[taxa1[i], taxa2[i]]<<-pcor_perm.pval[taxa2[i], taxa1[i]]<<-1-permutation_test(r_i$residuals, 
                                                                                   r_j$residuals)
        
      })
    })
    res<-list(pvalue=pcor_perm.pval, RunTime=elap_time)
    auc_obj <- precrec::auc(evalmod(scores = pcor_perm.pval[upper.tri(pcor_perm.pval)], 
                                    labels = true_graph[upper.tri(true_graph)]))
  }else if(method=='bPLS'){
