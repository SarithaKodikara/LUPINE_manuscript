...
load("Results/df_sen_pre_b_1.rds")
df_bpls1<-df_sen_pre
load("Results/df_sen_pre_b_2.rds")
df_bpls2<-df_sen_pre
load("Results/df_sen_pre_b_3.rds")
df_bpls3<-df_sen_pre
###plots for PLS

df_sen_pre_all<- rbind(df_bpls1,df_bpls2, df_bpls3)
df_sen_pre_all$Method<-factor(df_sen_pre_all$Method, 
                              levels = c( "bPLS_ncomp1", "bPLS_ncomp2","bPLS_ncomp3"), 
                              labels = c("LUPINE (ncomp=1)", "LUPINE (ncomp=2)", "LUPINE (ncomp=3)"))
...
pdf("Figures/n120_bpls_ncomp.pdf",
    width=8, height=8)
set.seed(12345)
op <- par(mar = rep(0, 4))
...
data_long %>%
  ...
  geom_boxplot() +
  ylim(0.2,1)+
  scale_fill_manual(values=c("deepskyblue","turquoise", "darkseagreen"))+
...
pdf("Figures/legend_bpls_ncomp.pdf",
    width=9, height=0.5)
op <- par(mar = rep(0, 4))
grid.draw(legend) 
par(op)
