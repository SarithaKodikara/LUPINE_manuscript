...
auc_res<-function( iter, day, method='sparcc'){
  ...
  if(method=='PCA_ncomp1'){
  ...
  }else if(method=='PCA_ncomp2'){
    elap_time<-system.time({
      pcor<-matrix(NA, nrow = nOTU,ncol = nOTU)
      pcor.pval<-matrix(NA, nrow = nOTU,ncol = nOTU)
      
      X_1<-newSimData[[iter]][,, day]
      X_1_pca<-pca(X_1, ncomp=2, scale=TRUE)
      loadings_m<-matrix(c(X_1_pca$loadings$X),ncol = 2)
    ...
    auc_obj <- precrec::auc(evalmod(scores = pcor.pval[upper.tri(pcor.pval)], 
                                    labels = true_graph[upper.tri(true_graph)]))
  }else if(method=='PCA_ncomp3'){
    elap_time<-system.time({
      pcor<-matrix(NA, nrow = nOTU,ncol = nOTU)
      pcor.pval<-matrix(NA, nrow = nOTU,ncol = nOTU)
      
      X_1<-newSimData[[iter]][,, day]
      X_1_pca<-pca(X_1, ncomp=3, scale=TRUE)
      loadings_m<-matrix(c(X_1_pca$loadings$X),ncol = 3)
    ...
    elap_time<-system.time({
      ...
      if(day==2){
        res_netPLS1<-pls(d1$X1, X_t, 
                         ncomp=2, max.iter =1000)
        
      }else{
        res_netPLS1<-block.pls(d1, X_t, ncomp=2, max.iter =1000)
        
      }
      
      loadings_m<-matrix(c(res_netPLS1$loadings$Y),ncol = 2)
    ...
    elap_time<-system.time({
      ...
      if(day==2){
        res_netPLS1<-pls(d1$X1, X_t, 
                         ncomp=3, max.iter =1000)
        
      }else{
        res_netPLS1<-block.pls(d1, X_t, ncomp=3, max.iter =1000)
        
      }
      
      loadings_m<-matrix(c(res_netPLS1$loadings$Y),ncol = 3)
